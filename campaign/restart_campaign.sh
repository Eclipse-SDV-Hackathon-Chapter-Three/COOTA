...
HTTP_CODE=$(curl -s -w "%{http_code}" -X DELETE -H "Authorization: Bearer $TOKEN" "${SYMPHONY_API_URL}activations/registry/fleet-app-campaign-activation" -o delete_response.json)
